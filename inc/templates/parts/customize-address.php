<script type="text/template" id="justwpforms-customize-address-template">
	...
	<?php do_action( 'justwpforms_part_customize_address_before_options' ); ?>
	<p>
		<label for="<%= instance.id %>_mode"><?php _e( 'Mode', 'justwpforms' ); ?></label>
		<select id="<%= instance.id %>_mode" name="mode" data-bind="mode" class="widefat">
			<option value="simple"<%= (instance.mode == 'simple') ? ' selected' : '' %>><?php _e( 'Full', 'justwpforms' ); ?></option>
			<option value="country-city"<%= (instance.mode == 'country-city') ? ' selected' : '' %>><?php _e( 'Country and city', 'justwpforms' ); ?></option>
			<option value="country"<%= (instance.mode == 'country') ? ' selected' : '' %>><?php _e( 'Country only', 'justwpforms' ); ?></option>
		</select>
	</p>

	<?php do_action( 'justwpforms_part_customize_address_after_options' ); ?>

	<?php do_action( 'justwpforms_part_customize_address_before_advanced_options' ); ?>
	...
	<?php do_action( 'justwpforms_part_customize_address_after_advanced_options' ); ?>
