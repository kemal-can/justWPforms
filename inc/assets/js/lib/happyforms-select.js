(function ($) {
	...
	var justwpformsSelect = function( el, options ) {
		this.el = el;
		this.$el = $( this.el );
		...
		this.$placeholder = $( 'li.justwpforms-custom-select-dropdown__placeholder', this.$select );
		this.$noResults = $( 'li.justwpforms-custom-select-dropdown__not-found', this.$select );
	...
	justwpformsSelect.prototype.init = function() {
	    ...
	    this.$el.on( 'justwpformsSelect.submitted', this.navigateDropdown.bind( this ) );
	}

	justwpformsSelect.prototype.mouseClickDown = function( e ) {
		var parent = $( e.target ).parents( '.justwpforms-form');
	...
	justwpformsSelect.prototype.handleClick = function( e ) {
		e.stopPropagation();
	...
	justwpformsSelect.prototype.handleFocus = function( e ) {
		this.keepFocus = true;
	...
	justwpformsSelect.prototype.handleBlur = function( e ) {
		var $parent = this.$clicked.parents( '.justwpforms-custom-select-dropdown' );
	...
	justwpformsSelect.prototype.hideAllInstances = function() {
		$( '.justwpforms-custom-select-dropdown' ).not( this.$select ).hide();
	}

	justwpformsSelect.prototype.toggleDropdown = function() {
	...
	justwpformsSelect.prototype.onOutsideClick = function( e ) {
		if ( ! this.el.contains(e.target).length && this.$select.is( ':visible' ) ) {
			this.hideDropdown();
		}
	}

	justwpformsSelect.prototype.hideDropdown = function() {
		if ( ! this.keepFocus ) {
			this.$select.hide();
			...
			if ( 'true' === this.searchable ) {
				...
				$( 'li.justwpforms-dropdown-item', this.$select ).show();
				this.$noResults.hide();
			}
		}
	}

	justwpformsSelect.prototype.onItemSelect = function( e ) {
		e.stopPropagation();
	...
	justwpformsSelect.prototype.handleKeyDown = function( e ) {
		switch ( e.keyCode ) {
			case 38:
			case 40:
				e.preventDefault();
	...
	justwpformsSelect.prototype.navigateDropdown = function ( e, windowEvent ) {
		this.handleKeyUp( windowEvent );
	}

	justwpformsSelect.prototype.handleKeyUp = function( e ) {
		switch ( e.keyCode ) {
			case 27:
			this.keepFocus = false;
				this.hideDropdown();
	...
	justwpformsSelect.prototype.onChange = function( e ) {
	...
	justwpformsSelect.prototype.getSuggestions = function() {
		var self = this;
	...
	justwpformsSelect.prototype.handleAutocomplete = function( data, partial, trigger ) {
		if ( ! data.length ) {
			return;
		}
		...
		var liTemplate = '<li class="justwpforms-dropdown-item justwpforms-custom-select-dropdown__item" data-value="%value%" data-label="">%value%</li>';
	...
	justwpformsSelect.prototype.bindAutocomplete = function() {
	...
	justwpformsSelect.prototype.handleAutocompleteInputClick = function( e ) {
		e.stopPropagation();

		this.toggleDropdown();
	}

	justwpformsSelect.prototype.handleAutocompleteInputKeyUp = function( e ) {
		if ( 13 === e.keyCode ) {
			return;
		}
	...
	justwpformsSelect.prototype.searchOptions = function() {
		var self = this;
		var value = this.$el.val();
		var $allItems = $( 'li.justwpforms-dropdown-item', this.$select );
		var foundItems = 0;
	...
	justwpformsSelect.prototype.gotoOption = function( e ) {
	...
	justwpformsSelect.prototype.navigateOptions = function( direction ) {
		if ( ! direction ) {
			return;
		}
	...
	justwpformsSelect.prototype.clearOptions = function() {
		var $allItems = $( 'li.justwpforms-dropdown-item', this.$select );

		$allItems.remove();
	}

	justwpformsSelect.prototype.setValue = function( value ) {
		...
		this.$input.trigger( 'justwpforms-change' );
	}

	justwpformsSelect.prototype.getValue = function() {
		return this.$el.val();
	}

	justwpformsSelect.prototype.setDefaults = function( value, label ) {
			this.defaultValue = value;
			this.defaultLabel = label;
	}

	$.fn.justwpformsSelect = function( method ) {
		this.each(function() {
			if ( 'object' === typeof method ) {
				$.data( this, 'justwpformselect', new justwpformsSelect( this, method ) );
			} else {
				var methods = $.data( this, 'justwpformselect' );
