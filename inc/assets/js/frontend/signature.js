( function( $ ) {
	...
	justwpforms.parts.signature = {
		init: function() {
			...
			if ( 'draw' === this.$el.attr( 'data-justwpforms-signature-type' ) ) {
		...
		serialize: function() {
			var serialized = justwpforms.parts.base.serialize.apply( this, arguments );

			if ( 'draw' === this.$el.attr( 'data-justwpforms-signature-type' ) ) {
				serialized.push( {
					name: this.$signatureSVG.attr( 'data-justwpforms-name' ),
		...
		initDrawingArea: function() {
			this.$signatureDataInput = $( '[data-justwpforms-signature-data]', this.$el );
			this.$signatureAreaContainer = $( '.justwpforms--signature-area--container', this.$el );
			this.$signatureArea = $( '.justwpforms--signature-area', this.$el );
			this.$signatureSVG = $( 'svg', this.$signatureAreaContainer );
			this.$signaturePath = $( 'path', this.$signatureSVG );
			this.$signaturePathDataInput = $( '[data-justwpforms-path-data]', this.$el );
			this.$signatureRasterDataInput = $( '[data-justwpforms-raster-data]', this.$el );

			this.formId = $( '[name="justwpforms_form_id"]', this.$form ).val();
			this.partId = this.$el.attr( 'data-justwpforms-id' );
			this.signaturePath = '';
			this.signaturePoints = [];

			this.$startDrawingButton = $( '.justwpforms--signature-area--start-drawing', this.$el );
			this.$clearDrawingButton = $( '.justwpforms--signature-area--clear-drawing', this.$el );
			this.$doneDrawingButton = $( '.justwpforms--signature-area--done-drawing', this.$el );
			this.$editDrawingButton = $( '.justwpforms--signature-area--edit-drawing', this.$el );
