(function ($) {
	...
	justwpforms.parts.date = {
		init: function () {
			this.type = this.$el.data( 'justwpforms-type' );
			this.spinnerInterval = '';
			this.spinnerIntervalLength = 150;
			this.required = 'undefined' === typeof this.$el.data( 'justwpforms-required' ) ? false : true;
			this.$input = $('input, select', this.$el);
			this.$numbers = $('.justwpforms-part-date__time-input input', this.$el);
			this.$spinnerUpArrow = $('.justwpforms-spinner-arrow--up', this.$el);
			this.$spinnerDownArrow = $('.justwpforms-spinner-arrow--down', this.$el);
