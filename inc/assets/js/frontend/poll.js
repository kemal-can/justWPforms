( function( $ ) {
	...
	justwpforms.parts.poll = {
		init: function() {
			...
			// poll links
			$( 'button.justwpforms-poll__show-results', this.$el ).on( 'click', this.onShowResultsClick.bind( this ) );
			$( 'button.justwpforms-poll__back-to-poll', this.$el ).on( 'click', this.onBackToPollClick.bind( this ) );
		...
		onPollOptionChange: function( e ) {
			...
			if ( this.$el.hasClass( 'justwpforms-poll--allow-multiple' ) ) {
				otherControlType = 'checkbox';
			}

			var $otherControl = $( '.justwpforms-part-option--other input[type='+ otherControlType +']', this.$el );
			...
			if ( $otherControl.length && $otherControl.is( ':checked' ) ) {
				...
				if ( '' === this.$el.data( 'justwpforms-required' ) ) {
