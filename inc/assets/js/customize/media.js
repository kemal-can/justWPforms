( function( $, Backbone ) {
	...
	var justwpformsMediaHandle = function( el, model, options ) {
		this.el = el;
		this.$el = $( this.el );
		this.model = model;
		...
		this.$uploadButton = $( '.justwpforms-upload-button', this.$el );
		this.$changeButton = $( '.justwpforms-change-button', this.$el );
		this.$removeButton = $( '.justwpforms-remove-button', this.$el );
	...
	justwpformsMediaHandle.prototype.onUploadButtonClick = function( e ) {
		e.preventDefault();

		this.openOverlay();
	}

	justwpformsMediaHandle.prototype.onChangeButtonClick = function( e ) {
		e.preventDefault();

		this.openOverlay();
	}

	justwpformsMediaHandle.prototype.onRemoveButtonClick = function( e ) {
		e.preventDefault();

		this.removeMedia();
	}

	justwpformsMediaHandle.prototype.openOverlay = function() {
		var self = this;
		...
		this.mediaFileFrame.on( 'select', function() {
			...
			switch ( attachment.type ) {
				case 'image':
					$( '.justwpforms-upload-preview' ).removeClass( 'show' );
	...
	justwpformsMediaHandle.prototype.removeMedia = function() {
	...
	$.fn.justwpformsMediaHandle = function( model, options ) {
		this.each(function() {
			$.data( this, 'justwpformsMediaHandle', new justwpformsMediaHandle( this, model, options ) );
		} );
	}
