( function( $, _, Backbone, api, settings ) {
	...
	justwpforms.classes.models.parts.date = justwpforms.classes.models.Part.extend( {
		defaults: function() {
			return _.extend(
				{},
	...
	justwpforms.classes.views.parts.date = justwpforms.classes.views.Part.extend( {
		template: '#justwpforms-customize-date-template',
	...
	justwpforms.previewer = _.extend( justwpforms.previewer, {
		onDateDateTypeChangeCallback: function( id, html, options ) {
			...
			switch ( dateType ) {
				case 'date':
					$part.removeClass( 'justwpforms-part-date--datetime' );
					$part.removeClass( 'justwpforms-part-date--time' );
					break;

				case 'datetime':
					$part.removeClass( 'justwpforms-part-date--date' );
					$part.removeClass( 'justwpforms-part-date--time' );
					break;
			...
			$part.addClass( 'justwpforms-part-date--' + dateType );
		},

		onDateTimeFormatChange: function( id, html, options ) {
			var part = this.getPartModel( id );
			var $part = this.getPartElement( html );
			var $period = this.$( '.justwpforms-part-date__time-input--period', $part );
