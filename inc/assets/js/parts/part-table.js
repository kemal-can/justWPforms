( function( $, _, Backbone, api, settings, partSettings ) {
	...
	justwpforms.classes.models.parts.table = justwpforms.classes.models.Part.extend( {
		defaults: function() {
			return _.extend(
				{},
	...
	var ColumnView = Backbone.View.extend( {
		template: '#customize-justwpforms-table-column-template',
		...
		onAdvancedColumnClick: function( e ) {
			...
			$( '.justwpforms-part-item-advanced', this.$el ).slideToggle( 300, function() {
	...
	var RowView = Backbone.View.extend( {
		template: '#customize-justwpforms-table-row-template',
	...
	justwpforms.classes.views.parts.table = justwpforms.classes.views.Part.extend( {
		template: '#customize-justwpforms-table-template',

		events: _.extend( {}, justwpforms.classes.views.Part.prototype.events, {
			'click [data-justwpforms-tab]': 'onTabClick',
		...
		onAddColumnClick: function( e ) {
			...
			var columnID = justwpforms.utils.uniqueId( this.model.get( 'id' ) + '_column_', this.model.get( 'columns' ) );
		...
		onTabClick: function( e ) {
			...
			var tabClass = $link.attr( 'data-justwpforms-tab' );
	...
	justwpforms.previewer = _.extend( justwpforms.previewer, {
		...
		onTableColumnDeleteCallback: function( id, html, options ) {
			...
			this.$( '.justwpforms-table__cell:nth-child(' + columnIndex + ')', $part ).remove();
		},
		...
		onTableColumnDescriptionChangeCallback: function( id, html, options ) {
			...
			this.$( '.justwpforms-part-option__description', $column ).text( column.get( 'description' ) );
		},
		...
		onTableRowDescriptionChangeCallback: function( id, html, options ) {
			...
			this.$( '.justwpforms-part-option__description', $row ).text( row.get( 'description' ) );
		},
		...
		onTableRowLabelChangeCallback: function( id, html, options ) {
			...
			this.$( '.justwpforms-table__row-label', $row ).text( row.get('label') );
		},
		...
		onTableColumnDefaultChangeCallback: function( id, html, options ) {
			...
			this.$( '.justwpforms-table__row--body .justwpforms-table__cell:nth-child(' + columnIndex + ') input', $part ).prop( 'checked', isDefault );
		},
	} );

} ) ( jQuery, _, Backbone, wp.customize, _justwpformsSettings, _justwpformsTableSettings );
