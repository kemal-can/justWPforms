( function( $, Backbone ) {
	...
	var justwpformsExportView = Backbone.View.extend( {
		el: '#justwpforms-export-form',

		events: {
			'change #justwpforms-export-select-action-select': 'onActionSelectChange',
			'change #justwpforms-export-select-form-select': 'onFormDropdownChange',
			'change #justwpforms-export-import-upload-stub': 'onFileAdd',
			'submit': 'onFormSubmit'
		},
		...
		initialize: function() {
			...
			this.$actionSelect = $( '#justwpforms-export-select-action-select', this.$el );
			this.$importContainer = $( '#justwpforms-export-section-import', this.$el );
			this.$fileStub = $( '#justwpforms-export-import-upload-stub', this.$el );
			this.importAttachmentID = 0;
			this.ajaxImportAction = 'justwpforms_import_form';
		...
		bindUploaderEvents: function() {
			...
			uploaderMultiPartParams.is_justwpforms_export = true;
		...
		resetFormDropdown: function() {
			$( '#justwpforms-export-select-form-select', this.$el ).val( '' );
		},
			...
			if ( 'import' === this.model.get( 'action' ) ) {
				...
				$.ajax({
					...
					success: function( response ) {
						var $importMessages = $( '#justwpforms-import-messages', self.$importContainer );

						if ( response.success ) {
							self.$el.addClass( 'import-success' );
							var $div = $( '<div />' ).attr( {
								class: 'justwpforms-export-message success'
							} );
							...
							$( 'p', $importMessages ).addClass( 'justwpforms-export-message error' );
						}
	...
	var justwpformsExportModel = Backbone.Model.extend( {
		action: ''
	} );

	$( function() {
		new justwpformsExportView( {
			model: new justwpformsExportModel
		} );
	} );
