...
class justwpforms_Form_Part_Library {
	...
	private $standard_parts = array(
		'justwpforms_Part_SingleLineText' => 'class-part-single-line-text',
		'justwpforms_Part_MultiLineText' => 'class-part-multi-line-text',
		'justwpforms_Part_Email' => 'class-part-email',
		'justwpforms_Part_Radio' => 'class-part-radio',
		'justwpforms_Part_Checkbox' => 'class-part-checkbox',
		'justwpforms_Part_Select' => 'class-part-select',
		'justwpforms_Part_Number' => 'class-part-number',
	);
	...
	public function __construct() {
		require_once( justwpforms_get_core_folder() . '/classes/class-form-part.php' );

		foreach ( $this->standard_parts as $part_class => $part_file ) {
			require_once( justwpforms_get_core_folder() . "/classes/parts/{$part_file}.php" );
	...
	public function register_part( $part, $index = -1 ) {
		$key = $part instanceof justwpforms_Form_Part ? $this->hash_object( $part ) : $part;
		$value = $part instanceof justwpforms_Form_Part ? $part : new $part();
	...
	public function get_part( $type ) {
		...
		$part = apply_filters( 'justwpforms_library_get_part_' . $type, $part );
	...
	public function customize_enqueue_scripts() {
		wp_register_script(
			'part-field-choice',
			justwpforms_get_plugin_url() . 'core/assets/js/parts/part-field-choice.js',
			'justwpforms-customize', justwpforms_get_version(), true
		);

		$deps = array( 'justwpforms-customize', 'part-field-choice' );
			...
			if ( array_key_exists( 'sanitize', $field_settings ) ) {
				...
				if ( is_callable( $sanitize_callback ) ) {
					...
					$error = new WP_Error( 'part', sprintf(
						__( 'Missing validation callback for field %s', 'justwpforms' ),
						$field_name
					) );
	...
	public function html_part_class( $class, $part_data, $form_data ) {
		$class[] = 'justwpforms-form__part';
		$class[] = 'justwpforms-part justwpforms-part--' . $part_data['type'];

		if ( isset( $part_data['width'] ) ) {
			$class[] = 'justwpforms-part--width-' . esc_attr( $part_data['width'] );
		}

		if ( isset( $part_data['label_placement'] ) ) {
			$class[] = 'justwpforms-part--label-' . esc_attr( $part_data['label_placement'] );
		}
	...
	public function html_part_data_attributes( $attributes, $part_data, $form_data ) {
		$attributes['justwpforms-type'] = $part_data['type'];
		$attributes['justwpforms-id'] = $part_data['id'];

		if ( isset( $part_data['required'] ) && 1 === intval( $part_data['required'] ) ) {
			$attributes['justwpforms-required'] = '';
		}
...
if ( ! function_exists( 'justwpforms_get_part_library' ) ):

function justwpforms_get_part_library() {
	return justwpforms_Form_Part_Library::instance();
}

endif;

justwpforms_get_part_library();
