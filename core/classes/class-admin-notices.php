...
class justwpforms_Admin_Notices {
	 ...
	 * @since 1.0
	 *
	 * @return justwpforms_Admin_Notices
	 */
	...
	public function hook() {
		...
		add_action( 'justwpforms_preview_notices', array( $this, 'display_preview_notices' ) );
		add_action( 'justwpforms_form_before', array( $this, 'handle_preview_notices' ), 20 );
		add_action( 'wp_ajax_justwpforms_hide_notice', array( $this, 'handle_ajax' ) );
	}
	...
	public function display( $screen_id = '' ) {
		...
		foreach ( $notices as $id => $notice ) {
			...
			$default_classes = array( 'notice', 'justwpforms-notice' );
			...
			$nonce = ( $dismissible && ! $onetime ) ? ' data-nonce="' . esc_attr( wp_create_nonce( 'justwpforms_dismiss_' . $id ) ) . '"' : '';
			...
			?>
			<div id="justwpforms-notice-<?php echo esc_attr( $id ); ?>" class="<?php echo esc_attr( $classes_string ); ?>"<?php echo $nonce; ?>>
				<?php if ( isset( $notice['title'] ) ) : ?>
					<h2 class="justwpforms-notice__title"><?php echo $notice['title']; ?></h2>
				<?php endif; ?>
				<?php
				if ( 'custom' !== $type ) :
				...
				?>
				<?php if ( $dismissible && 'custom' === $type && 'justwpforms-preview' === $screen_id ) : ?>
					<a href="#" class="justwpforms-dismiss-notice"></a>
				<?php endif; ?>
			</div>
			<?php
		}
	...
	public function handle_preview_notices( $form ) {
		if ( justwpforms_is_preview() ) {
			...
			$notices = $this->get_notices( 'justwpforms-preview' );
	...
	public function get_user_transient_id( WP_User $user = null ) {
		...
		return 'justwpforms_admin_notices_' . md5( $user->user_login );
	}
	 ...
	 * @since 1.0
	 *
	 * @hooked action wp_ajax_justwpforms_hide_notice
	 *
	 * @return void
	 */
	public function handle_ajax() {
		// Only run this during an Ajax request.
		if ( 'wp_ajax_justwpforms_hide_notice' !== current_action() ) {
			return;
		}
		...
		if ( ! defined( 'DOING_AJAX' ) ||
			...
			false === $nid ||
			false === $nonce ||
			! wp_verify_nonce( $nonce, 'justwpforms_dismiss_' . $nid ) ) {
	...
	public function get_dismissed_notices( $user_id ) {
		$dismissed = get_user_meta( $user_id, 'justwpforms-dismissed-notices', true );
	...
	public function update_dismissed_notices( $user_id, array $notices ) {
		return update_user_meta( $user_id, 'justwpforms-dismissed-notices', $notices );
	}

}

if ( ! function_exists( 'justwpforms_get_admin_notices' ) ):
/**
 * Get the justwpforms_Admin_Notices class instance.
 *
 * @since 1.0
 *
 * @return justwpforms_Admin_Notices
 */
function justwpforms_get_admin_notices() {
	return justwpforms_Admin_Notices::instance();
}

endif;

/**
 * Initialize the justwpforms_Admin_Notices class immediately.
 */
justwpforms_get_admin_notices();
