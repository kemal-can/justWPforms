( function( $, _, Backbone, api, settings ) {
	...
	justwpforms.classes.models.Option = Backbone.Model.extend( {
		defaults: {
			type: 'option',
		}
	} );

	justwpforms.classes.views.Option = Backbone.View.extend( {
		events: {
			'click .justwpforms-delete-item': 'onDeleteClick',
			'click .justwpforms-duplicate-item': 'onDuplicateClick',
			'click .justwpforms-advanced-option-action': 'onAdvancedOptionClick',
		},
		...
		closeOpenWidgets: function( $currentElement ) {
			var $openWidgets = $( '.justwpforms-choice-item-widget').not( $currentElement );

			$openWidgets.removeClass( 'justwpforms-widget-choice-expanded' );
			$openWidgets.find( '.justwpforms-part-item-advanced' ).slideUp( 200, function() {
		...
		onAddOpenWidget: function( e ) {
			...
			$el.toggleClass( 'justwpforms-widget-choice-expanded' );
			this.closeOpenWidgets( $el );

			$( '.justwpforms-part-item-advanced', this.$el ).slideToggle( 200, function() {
				if ( $el.hasClass( 'justwpforms-widget-choice-expanded' ) ) {
		...
		onDeleteClick: function( e ) {
			...
			$( '.justwpforms-part-item-advanced', this.$el ).slideUp( 'fast', function() {
	...
	justwpforms.classes.views.OptionHeading = justwpforms.classes.views.Option.extend( {
		events: _.extend( {}, justwpforms.classes.views.Option.prototype.events, {
	...
	justwpforms.classes.views.OptionItem = justwpforms.classes.views.Option.extend( {
		events: _.extend( {}, justwpforms.classes.views.Option.prototype.events, {
		...
		initialize: function( options ) {
			justwpforms.classes.views.Option.prototype.initialize.apply( this, arguments );
	...
	justwpforms.classes.views.ChoiceField = justwpforms.classes.views.Part.extend( {
		...
		onReady: function() {
			...
			$( '.option-list', this.$el ).sortable( {
				handle: '.justwpforms-part-item-handle',
				helper: 'clone',
		...
		getOptionItemView: function( optionModel, options ) {
			var view = new justwpforms.classes.views.OptionItem( _.extend( {
				model: optionModel,
				part: this.model,
			}, options ) );
		...
		getOptionHeadingView: function( optionModel, options ) {
			var view = new justwpforms.classes.views.OptionHeading( _.extend( {
				model: optionModel,
				part: this.model,
			}, options ) );
		...
		onOptionViewAdd: function( viewModel, collection, options ) {
			...
			if ( 'undefined' === typeof( options.index ) ) {
				...
				$( '.justwpforms-choice-item-widget:nth-child(' + options.index + ')', this.$el ).after( optionView.render().$el );
			}
		...
		closeAllItemWidget: function() {
			var $openWidgets = $( '.justwpforms-choice-item-widget', this.$el );
	...
	justwpforms.previewer = _.extend( justwpforms.previewer, {
		...
		onOptionDescriptionChangeCallback: function( id, html, options ) {
			...
			this.$( '.justwpforms-part-option__description', $option ).text( option.get( 'description' ) );
		},
