( function( $ ) {
	...
	justwpforms.parts.select = {
		...
		onBlur: function() {
			var $otherinput = $( '.justwpforms-part-option--other input[type=text]', this.$el );
		...
		serialize: function() {
			...
			var serialized = this.$input.map( function( i, select ) {
				...
				if ( 999 == $select.val() ) {
					$customInput = $( ' .justwpforms-part-option--other input[type=text]', self.$el );
				}
