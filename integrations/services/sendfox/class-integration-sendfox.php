...
class justwpforms_Integration_SendFox {
	...
	public function __construct() {
		$this->service = justwpforms_get_integrations()->get_service( 'sendfox' );
	}
	...
	public function add_part_controls() {
		...
		<p>
			<label for="<%= instance.id %>_sendfox_field"><?php _e( 'Map field to SendFox field', 'justwpforms' ); ?></label>
			<select class="widefat justwpforms-client-updated" data-bind="sendfox_field" data-var="_justwpformsSendFoxData" data-var-prop="fields">
	...
	public function handle_submit( $submission_id ) {
		...
		foreach ( $form['parts'] as $part ) {
			if ( empty( $part['sendfox_field'] ) ) {
				continue;
			}

			$fields[$part['sendfox_field']] = $submission['parts'][$part['id']];
		}
		...
		if ( 'unsubscribe' === justwpforms_get_form_property( $form, "{$this->service->id}_subscribe_status" ) ) {
			$data['list_id'] = justwpforms_get_form_property( $form, 'sendfox_list_unsubscribe' );
			$this->service->unsubscribe( $data );
		} else {
			$data['list_id'] = justwpforms_get_form_property( $form, 'sendfox_list' );
...
if ( ! function_exists( 'justwpforms_get_integration_sendfox' ) ):

function justwpforms_get_integration_sendfox() {
	$instance = justwpforms_Integration_SendFox::instance();
	$instance->hook();
...
justwpforms_get_integration_sendfox();
