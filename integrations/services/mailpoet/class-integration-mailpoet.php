...
class justwpforms_Integration_MailPoet {
	...
	public function __construct() {
		$this->service = justwpforms_get_integrations()->get_service( 'mailpoet' );
	}
	...
	public function meta_fields( $fields ) {
		...
		$fields["{$this->service->id}_send_confirmation_email"] = array(
			'default' => 1,
			'sanitize' => 'justwpforms_sanitize_checkbox'
		);
	...
	public function setup_controls( $controls ) {
		...
		$email_controls = array(
			...
			213 => array(
				...
				'label' => __( 'List to add submitters\'s email to', 'justwpforms' ),
				'options' => $lists_options,
				'allow_empty' => true,
			),
			214 => array(
				'type' => 'checkbox',
				'field' => "{$this->service->id}_send_confirmation_email",
				'label' => __( 'Send confirmation email', 'justwpforms' ),
			),
			215 => array(
				'type' => 'select',
				'field' => "{$this->service->id}_list_unsubscribe",
				'placeholder' => __( '— Select —', 'justwpforms' ),
				'label' => __( 'List to remove submitters\'s email from', 'justwpforms' ),
				'options' => $lists_options,
				'allow_empty' => true,
			),
			216 => array(
	...
	public function add_part_controls() {
		...
		<p>
			<label for="<%= instance.id %>_mailpoet_field"><?php _e( 'Map field to MailPoet field', 'justwpforms' ); ?></label>
			<select class="widefat justwpforms-client-updated" data-bind="mailpoet_field" data-var="_justwpformsMailPoetData" data-var-prop="fields">
	...
	public function handle_submit( $submission_id ) {
		...
		foreach ( $form['parts'] as $part ) {
			if ( empty( $part['mailpoet_field'] ) ) {
				continue;
			}

			$fields[$part['mailpoet_field']] = $submission['parts'][$part['id']];
		}

		$data = array(
			'fields' => $fields,
			'send_confirmation_email' => $form['mailpoet_send_confirmation_email'],
		);

		if ( 'unsubscribe' === justwpforms_get_form_property( $form, "{$this->service->id}_subscribe_status" ) ) {
			$data['list_id'] = justwpforms_get_form_property( $form, 'mailpoet_list_unsubscribe' );
			$this->service->unsubscribe( $data );
		} else {
			$data['list_id'] = justwpforms_get_form_property( $form, 'mailpoet_list' );
...
if ( ! function_exists( 'justwpforms_get_integration_mailpoet' ) ):

function justwpforms_get_integration_mailpoet() {
	$instance = justwpforms_Integration_MailPoet::instance();
	$instance->hook();
...
justwpforms_get_integration_mailpoet();
