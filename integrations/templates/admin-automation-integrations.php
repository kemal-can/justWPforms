...
$service = $integrations->get_service( 'automation' );
...
?>
<p><?php _e( 'Connect forms with automation services.', 'justwpforms' ); ?></p>
<div class="widget-content has-service-selection <?php if ( $service->is_connected() ) : ?>authenticated<?php endif; ?>" data-active-service="<?php echo $active_service; ?>">
	...
	<form class="justwpforms-service hf-ajax-submit">
		...
		<input type="hidden" name="group" value="automation">

		<p>
			<label for="justwpforms_integrations_automation_service"><?php _e( 'Service:', 'justwpforms' ); ?></label>
			<select id="justwpforms_integrations_automation_service" name="services[]" class="widefat">
				<option value="">— <?php _e( 'Select', 'justwpforms' ); ?> —</option>

				<?php foreach( $groups['automation'] as $sub_service ) : ?>
		...
		<?php
		foreach ( $groups['automation'] as $sub_service ) {
